...
ANLStatus OISTReadDataFile_Muon2::mod_init()
{
  ...
  m_ADC.reset(new std::unique_ptr<int[]>[numASICs]);
  m_INDEX.reset(new std::unique_ptr<uint16_t[]>[numASICs]);
  m_CMN.reset(new int[numASICs]);
  ...
  for (auto& readoutModule: detectorManager->getReadoutModules()) {
    for (auto& section: readoutModule->getReadoutSections()) {
      ...
      m_ADC[iASIC].reset(new int[2*numChannels]);
...
ANLStatus OISTReadDataFile_Muon2::mod_ana()
{
  ...
  setTime(m_UnixTime);
  m_exptime = (unsigned long int) m_IntegralLiveTime;
